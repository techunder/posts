...
过程中会以当下流行的**本地智能体 [OpenClaw](https://github.com/openclaw/openclaw)** 为例。
...
以上例子的完整版会话上下文可以在「<a href="/images/docs/03-ai-agent-intro/openclaw-session.json" target="_blank">这里</a>」查看（文件大小 69 KB）

其中的 system prompt 部分可以在「<a href="/images/docs/03-ai-agent-intro/system-prompt.txt" target="_blank">这里</a>」查看（文件大小 32 KB）
...
[**MCP**](https://modelcontextprotocol.io/)（Model Context Protocol，模型上下文协议）是一个开放协议，旨在标准化 AI 助手与外部数据源、工具之间的通信方式。
...
> **Streamable HTTP** = HTTP POST（客户端发请求）+ SSE（Server-Sent Events 服务器流式推送，可选），两者结合实现了远程 MCP 通信的请求响应 + 服务端实时推送能力
...
在上一篇文章的[上下文长度](../01-llm/#%E4%B8%8A%E4%B8%8B%E6%96%87%E9%95%BF%E5%BA%A6)章节中，我们提到了每一个 LLM 都是有上下文长度限制。
...
即使 OpenClaw 的定位是本地 AI 助手，但它同时也是当前 AI 智能体工程领域第一梯队的实战应用，
