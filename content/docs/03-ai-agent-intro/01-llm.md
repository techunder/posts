...
<p>
  <img src="/images/docs/03-ai-agent-intro/neuron.png" alt="神经元" style="width: 20%; height: auto;">
  <img src="/images/docs/03-ai-agent-intro/brain.png" alt="大脑神经网络" style="width: 45%; height: auto;">
</p>

![计算机深度神经网络](/images/docs/03-ai-agent-intro/neural-network.png)
...
![Transformer模型架构](/images/docs/03-ai-agent-intro/transformer-model-architecture.png)
...
{{< asciinema
  cast="/images/docs/03-ai-agent-intro/stoken-spit-one-by-one.cast"
  loop=true
  autoplay=true
  speed=2 >}}
...
所谓**工具调用**（tool calling，也叫 function calling），就是 **LLM 跟据上下文判断用户意图，生成函数调用的结构化输出**。
...
> 这就是为什么明明你调用的不是 OpenAI 或 Anthropic 的模型，却在使用他们的 SDK 的原因。
> 只要设置了正确的 URL 和 API Key，服务端会做接口兼容
...
> LLM 也可以本地部署，例如使用 [Ollama](https://ollama.com/)，它提供了自己的接口格式
