...
所谓**工具调用**（tool calling / function calling），就是 LLM 可以跟据上下文判断用户意图，生成函数调用的代码。

> [!TIP]
> **编程语言是目前为止最精确的意图表达方式**，不是自然语言，不是图片，是代码。代码可以直接转换成行动，而自然语言还需要一个翻译层。

> [!TIP]
> LLM 的 tool calling 能力是 AI Agent 接管世界的开始，是这位“文科生”由文转理的原因。
