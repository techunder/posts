...
<div class="page-info">
  ...
  发布时间：2026-04-14 | 更新时间：2026-07-11
</div>
{{< katex />}}
...
<center>
<p>
  <img src="/images/docs/03-embedding/vector.jpeg" alt="向量在向量空间中的表示" style="width: 50%; height: auto;">
</p>
（图：向量在向量空间中的表示）
</center>
...
{{% details title="**余弦相似度**的数学推导过程" open=false %}}
...
\| \boldsymbol{x} \|
```
表示 $\boldsymbol{x}$ 的 $L_2$ Norm，代表一个向量距离坐标原点的距离，即**欧几里得距离**。
```katex
\begin{aligned}
...
常规的 embedding model 是为词库里的每一个 token 随机生成一个高维向量（vector），然后输入大量的文本库进行训练，通过神经网络的反向传播机制修正 vector 的每一个维度值。

但会有两个问题：
- 内存爆炸：当词库里的 token 数量很多，对应的 vector 会占用非常多的空间，内存开销大；
- OOV（Out-of-Vocabulary）：因为词库里的 token 需要预先分配好并训练，无法处理新增未知的 token。

为了改进这个工程问题，**Hash Embedding**（哈希嵌入）被提出。

**Deterministic Hash Embedding**：

对给定离散 token，先通过 `idx=Hash(token) mod bucket_size` 计算桶内（bucket）索引，按索引查表查得到桶内的某一个向量值。

桶（bucket）的结构类似下面这样：
| idx | vector |
|-----|--------|
| 1   | [1.5620e-01 -5.2500e-02  8.6000e-03 -3.2600e-02 -1.4270e-01 -6.3600e-02] |
| 2   | [4.2780e-01  2.1300e-01 -3.4000e-02  7.7500e-02 -6.3300e-02 -2.5110e-01] |
| 3   | [8.0300e-02 -2.1300e-02  1.1070e-01 -7.2100e-02  3.1060e-01 -9.7000e-03] |
| ... |   ...         ...         ...         ...         ...         ...        |
| n-1 | [2.6840e-01  1.0420e-01  1.7910e-01  2.0000e-04 -1.8340e-01  9.5300e-02] |
| n   | [2.0890e-01 -8.0000e-03 -7.6600e-02  1.8470e-01 -6.2700e-02 -3.2900e-02] |

> 桶内的每一向量值，代表通过大量文本训练得到的 embedding 值。

这样，当输入任意的 token，都可以映射为一个 idx，进而得到任一 embedding 值。

但这样做有一个问题，就是**碰撞**，不同的 token 哈希后，有可能会得到相同的 idx 值，进而得到相同的 embedding 值，哪怕两个不同语义的 token。训练时这两个不同语义的 token 会拉扯同一个 embedding 向量，会产生噪音，稀释相似度。

为了缓解碰撞的问题，工业上通常增大桶的容量来缓解。

**Deep Hash Embedding**：

还有另外一种办法，就是引入可训练的 MLP（Multi-Layer Perceptron，多层感知机）网络把哈希特征转换为最终 embedding。过程如下：
1. 多层固定哈希把 token 映射成一组数字特性（无参数，确定性）
2. 输入训练过的 MLP 网络把哈希特征转换为 embedding。

从通过查表获取 embedding 变为把哈希特征输入 MLP 网络获取 embedding，避开了碰撞带来的向量冲突。

通过以上的工程方法，很好地解决了内存爆炸和 OOV 问题。

> [!TIP]
> Hash Embedding 除了以上的工程方法之外，还有 **Stochastic Hash Embedding**（随机哈希嵌入），通过在哈希寻址环节引入随机变量，前向传播对哈希索引做随机扰动、随机加权，来降低碰撞、过拟合等影响。

> [!TIP]
> 传统的一对一查表 embedding 叫 **Standard Embedding** 或者 **Full Embedding**。

# 结语

区别于传统基于字符串匹配的工作模式，例如**正则表达式**（regexp）、**通配符匹配**、**全文检索**（Full-Text Search）等搜索技术，embedding 的语义特性，为软件打开了语义理解的大门，各种向量数据库纷纷涌现，从此，**不再需要逐字上的精确匹配，而是允许语义上的模糊提取**，这在知识库、RAG（Retrieval-Augmented Generation）等的关键底层技术。

Embedding 也是深度学习的标准输入格式，它把文字、图像、音频这些离散信号，变成神经网络能懂的连续向量，是**多模态**输入的编码器（Encoder）和特征提取器（Feature Extractor）。所以 embedding 是 AI 时代一块不可或缺的基石，是大语言模型的基础设施之一。

<p>
  <img src="/images/docs/03-embedding/embedding.png" alt="向量空间" style="width: 50%; height: auto;">
</p>
